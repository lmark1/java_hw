...
public class Example1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Primjer unosa korisnkia.");
		BlogUser user = primjerUnosaKorisnkia(emf);
		System.out.println("Primjer unosa korisnika - gotovo.");
		
		// 1. korak - stvaranje novog blog zapisa...
		// -----------------------------------------
		System.out.println("Dodajem blog zapis.");
		BlogEntry blogEntry = dodajZapis(emf, user.getId());
	...
	private static BlogUser primjerUnosaKorisnkia(EntityManagerFactory emf) {
		...
		user.setNick("ivo");
		user.setPasswordHash(Util.generateDigest("password"));
	...
	public static BlogEntry dodajZapis(EntityManagerFactory emf, Long userID) {
		EntityManager em = emf.createEntityManager();
		em.getTransaction().begin();
